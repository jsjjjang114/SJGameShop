...
<%
	...
	int count = gdao.getAllCount();
	ArrayList<GameBean> lists = gdao.getNewArticles(startRow, endRow); 
%>
<br>
...
<%
	if(count > 0){
		...
		if(startPage>10){
		%>
			<a href="mall_new.jsp?pageNum=<%=startPage-10 %>">이전</a>
		<%		
		}
		for(int i=startPage;i<=endPage;i++){
		%>
			<a href="mall_new.jsp?pageNum=<%=i %>"><%=i %></a>
		<%
		}
		if(endPage<pageCount){
		%>
			<a href="mall_new.jsp?pageNum=<%=startPage+10 %>">다음</a>
		<%
		}
	}
%>
