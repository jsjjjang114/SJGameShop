...
<%
	...
	int count = gdao.getDevCount(developer);
	ArrayList<GameBean> lists = gdao.getAllByDev(startRow,endRow,developer); 
%>   
<br> 
<div align="left" style="margin-left: 100px;"><font size="5px">'<%=developer %>'사 게임</font></div>
<hr width="800">
<table width="800"> 
<tr>
<%
...
<%
	if(count > 0){
		...
		if(startPage>10){
		%>
			<a href="mall_dev.jsp?pageNum=<%=startPage-10 %>&developer=<%=developer%>">이전</a>
		<%		
		}
		for(int i=startPage;i<=endPage;i++){
		%>
			<a href="mall_dev.jsp?pageNum=<%=i %>&developer=<%=developer%>"><%=i %></a>
		<%
		}
		if(endPage<pageCount){
		%>
			<a href="mall_dev.jsp?pageNum=<%=startPage+10 %>&developer=<%=developer%>">다음</a>
		<%
		}
	}
%>
